...
// .wrangler/tmp/bundle-pPz5Hf/checked-fetch.js
...
auth.post("/login", async (c) => {
  try {
    ...
    const token = await sign2(
      { userId: user.id, email: user.email, role: user.role, exp: Math.floor(Date.now() / 1e3) + 60 * 60 * 24 * 7 },
      c.env.JWT_SECRET
    );
    return c.json({
      success: true,
...
bookings.post("/", async (c) => {
  try {
    ...
    const result = await sql`
            ...
            VALUES (${userId}, ${style_id}, ${designer}, ${booking_date}, ${booking_time}, ${notes ?? ""}, ${ref_number ?? ""})
            RETURNING *
        `;
...
var supabaseAuth = /* @__PURE__ */ __name(() => {
  return async (c, next) => {
    ...
    const token = authHeader.split(" ")[1];
    try {
      const payload = await verify2(token, c.env.SUPABASE_JWT_SECRET, "HS256");
      if (payload && payload.sub) {
        c.set("userId", payload.sub);
        c.set("userEmail", payload.email);
        console.log(`\u{1F512} Authenticated User: ${payload.email}`);
      }
      await next();
    } catch (err) {
...
app.route("/api/styles", styles_default);
app.route("/api/designers", designers_default);
...
app.route("/api/admin/settings", admin_settings_default);
...
// .wrangler/tmp/bundle-pPz5Hf/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-pPz5Hf/middleware-loader.entry.ts
