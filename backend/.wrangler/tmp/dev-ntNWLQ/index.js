...
// .wrangler/tmp/bundle-9cKzyL/checked-fetch.js
...
auth.post("/register", async (c) => {
  try {
    ...
    const result = await sql`
        ...
        `;
    return c.json({ success: true, message: "\u{1F389} \uD68C\uC6D0\uAC00\uC785 \uC131\uACF5!", user: result[0] }, 201);
  } catch (err) {
...
styles.patch("/:id", async (c) => {
  try {
    ...
    const result = await sql`
            UPDATE hairstyles
            SET 
                img_url = ${body.img_url !== void 0 ? body.img_url : sql`img_url`},
                price = ${body.price !== void 0 ? Number(body.price) : sql`price`},
                name_ko = ${body.ko !== void 0 ? body.ko : sql`name_ko`},
                name_en = ${body.en !== void 0 ? body.en : sql`name_en`},
                category = ${body.category !== void 0 ? body.category : sql`category`},
                is_active = ${body.is_active !== void 0 ? body.is_active : sql`is_active`}
...
bookings.get("/available", async (c) => {
  try {
    ...
    const bookedTimes = booked.map((b2) => b2.booking_time.slice(0, 5));
...
bookings.post("/", async (c) => {
  try {
    ...
    const sql = getDb(c.env.DATABASE_URL);
    const userEmail = c.get("userEmail") || "unknown@sns.com";
    ...
    const result = await sql`
            ...
            RETURNING *
        `;
    return c.json({ success: true, message: "\uC608\uC57D\uC774 \uC644\uB8CC\uB418\uC5C8\uC2B5\uB2C8\uB2E4! \u{1F389}", data: result[0] }, 201);
  } catch (err) {
...
// src/routes/designers.ts
var designers = new Hono2();
...
var designers_default = designers;

// src/routes/admin_settings.ts
var adminSettings = new Hono2();
...
var admin_settings_default = adminSettings;

// src/middleware/supabaseAuth.ts
var supabaseAuth = /* @__PURE__ */ __name(() => {
  return async (c, next) => {
    const authHeader = c.req.header("Authorization");
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
    ...
    try {
      ...
      if (payload && payload.sub) {
        ...
        console.log(`\u{1F512} Authenticated User: ${payload.email} (ID: ${payload.sub})`);
      } else {
...
app.route("/api/:shopId/auth", auth_default);
app.route("/api/:shopId/styles", styles_default);
...
app.route("/api/:shopId/designers", designers_default);
...
app.route("/api/:shopId/admin/settings", admin_settings_default);
...
// .wrangler/tmp/bundle-9cKzyL/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-9cKzyL/middleware-loader.entry.ts
